"ues client"

function SecondDesign(){
return(
    <div className="text-black">
        <div className="hover:-translate-y-7 duration-1000 flex items-center justify-end mr-8">
            <div className="-ml-4">
                <h2 className="font-black text-2xl text-right">Headphones & Speakers</h2>
                <p className="w-80 pl-8">Experience uncompromising sound wherever you are through wired and wireless technology.</p>
            </div>
            <div className="py-8">
                    <img src="/Images/headphone.jpg" className="h-80 w-96" ></img>
            </div>
            </div>
        </div>
)
}

export default SecondDesign;